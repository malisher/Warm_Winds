...
# import pickle
#
# from yolo_folder.models.common import DetectMultiBackend
# from yolo_folder.utils.dataloaders import LoadImages
# from yolo_folder.utils.general import (check_img_size, non_max_suppression, scale_boxes)
# from yolo_folder.utils.torch_utils import select_device, smart_inference_mode
#
#
...
from src.yolo_folder.detect import run


def run_detector(images_folder, weight_path, classes_path):
    with open(classes_path, 'rb') as f:
        classes_price = pkl.load(f)

    results_price = run(
        weights=weight_path,
        source=images_folder,
        classes=classes_price,
        imgsz=config.det_imgsz,
        conf_thres=config.det_conf_thres,
        iou_thres=config.det_iou_thres,
        max_det=config.det_max_det,
    )
