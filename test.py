...
def parse_opt():
    ...
    parser.add_argument('--crossed_detector_weight_path', default='weights/crossed_best250624x.pt',
                        help='Crossed-out detector model path включая weight name and extension')
    parser.add_argument('--crossed_recognizer_weight_path',
                        ...
                        help='Crossed-out recognizer model path включая weight name and extension')
...
def main():
    ...
    regular_recognizer = load_model_CRNN2(args.recognizer_weight_path)
    ...
    for img_file in os.listdir(args.images_folder):
        if Path(img_file).suffix.lower() not in valid_extensions:
            continue

        img_path = os.path.join(args.images_folder, img_file)
        process_image(img_path, results_price, regular_recognizer, crossed_recognizer, converter, transformer,
                      output_folder, args.crossed_detector_weight_path, args.detector_classes_path)
